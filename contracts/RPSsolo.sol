...
contract RockPaperScissorsSolo is ZamaEthereumConfig {
    ...
    constructor(address _player) {
        ...
        Player memory player1 = Player({playerAddress: _player, move: NO_MOVE, submitted: false, score: ZERO});
        Player memory player2 = Player({playerAddress: address(this), move: NO_MOVE, submitted: false, score: ZERO});
    ...
    function submitMove(externalEuint8 playerMove, bytes calldata inputProof) public {
        ...
        if (msg.sender == game.player1.playerAddress) {
            require(!game.player1.submitted, "Player already submitted this round");
            game.player1.move = move;
            game.player1.submitted = true;
            FHE.allowThis(game.player1.move);
        } else {
            revert("Only registered player can submit moves");
        }

        // generate computer's move
        game.player2.move = FHE.randEuint8(2);
